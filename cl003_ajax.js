// PDF Analysis (Fixed)
$('#pdfanalysissubmit').submit(function (event) {
    event.preventDefault();
    const dropdownArg = $('#pdfanalysissubmit select[name="pdfanalysis_arg"]').val();
    const customCmd = $('#pdfanalysissubmit input[name="cmdline"]').val().trim();
    const moduleName = "pdfanalysis";
    let cmd;
    
    // Debug: Let's see what we're getting
    console.log("🔍 Debug - dropdownArg:", dropdownArg);
    console.log("🔍 Debug - customCmd:", `"${customCmd}"`);
    console.log("🔍 Debug - customCmd length:", customCmd.length);
    
    if (dropdownArg === 'custom') {
        // Check if custom command is provided
        if (customCmd && customCmd.length > 0) {
            cmd = `${moduleName} --custom "${customCmd}"`;
        } else {
            alert('Please enter a custom command');
            return; // Stop submission if no custom command
        }
    } else {
        cmd = `${moduleName} --${dropdownArg}`;
    }
    
    console.log("📤 Final CMD:", cmd); // Debug only
    
    $.ajax({
        url: "{% url 'run-module' project=project %}",
        type: 'post',
        dataType: 'html',
        data: {
            module: moduleName,
            file_hash: "{{ malware.sha256 }}",
            cmdline: cmd,
            csrfmiddlewaretoken: '{{ csrf_token }}'
        },
        beforeSend: function () {
            var target = document.getElementById('spin_loader');
            var spinner = new Spinner(opts).spin(target);
            $(target).data('spinner', spinner);
        },
        complete: function () {
            $('#spin_loader').data('spinner').stop();
        },
        success: function (response, textStatus, jqXHR) {
            $('#pdfresponse').prepend(response);
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.log('error(s):' + textStatus, errorThrown);
        }
    });
});
